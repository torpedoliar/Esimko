...
<?php $__env->startSection('js'); ?>
    <script>
        let to, _token = '<?php echo e(csrf_token()); ?>', data = [], selected_page = 1;
        ...
        let search_data = (page = 1) => {
            ...
            $.post("<?php echo e(url('keuangan/jurnal/search')); ?>", {_token, bulan, bulan_akhir, paginate: 10, page: selected_page}, (result) => {
                $table.html(result);
            }).fail((xhr) => {
        ...
        let info = (id = '') => {
            $.get("<?php echo e(url('keuangan/jurnal')); ?>/" + (id === '' ? 'create' : (id + '/edit')), (result) => {
                $info.html(result);
                $jurnal.hide();
            }).fail((xhr) => {
        ...
        let delete_data = (id) => {
            Swal.fire({
                ...
                if (result.value) {
                    $.post("<?php echo e(url('keuangan/jurnal')); ?>/" + id, {_method: 'delete', _token}, () => {
                        discard();
                    }).fail((xhr) => {
                        $info.html(xhr.responseText);
                    });
        ...
        let export_excel = () => {
            let bulan = $('#bulan').val();
            let bulan_akhir = $('#bulan_akhir').val();
            window.open("<?php echo e(url('keuangan/jurnal/export')); ?>?bulan=" + bulan + '&bulan_akhir=' + bulan_akhir, '_blank');
        }
...
<?php echo $__env->make('layouts.admin', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH /var/www/html/resources/views/keuangan/jurnal/index.blade.php ENDPATH**/ ?>