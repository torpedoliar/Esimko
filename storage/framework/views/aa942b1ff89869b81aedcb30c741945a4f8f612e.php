    ...
    <div class="container-fluid p-0 w-100 d-flex flex-row shadow-sm" style="height: calc(100vh - 155px)">
                ...
                <div style="width: 350px;">
                    <select type="text" id="fid_supplier" name="fid_supplier" class="form-control select2">
                        ...
                        <?php $__currentLoopData = $supplier; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $value): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                            <option value="<?php echo e($value->id); ?>" <?php echo e(($pembelian->fid_supplier ?? '') == $value->id ? 'selected' : ''); ?>><?php echo e($value->nama_supplier); ?></option>
        ...
        <div class="w-25 position-relative" style="background-color: #dcdde1;padding: 20px;">
            <div class="form-group">
                <h3><small>No. Pembelian</small><br><span id="no_pembelian"><?php echo e(!empty($pembelian) ? $pembelian->no_pembelian : '-'); ?></span></h3>
            </div>
            <hr>
            <div class="form-group">
                ...
                <div style="display:flex">
                    <div class="input-group" style="width:150px">
                        <input type="text" name="diskon_persen" class="form-control autonumeric" data-a-dec="," data-a-sep="." id="diskon_persen" value="<?php echo e($pembelian->diskon_persen ?? '0'); ?>">
                    ...
                    <div class="input-group">
                        ...
                        <input type="text" name="diskon_nominal" class="form-control autonumeric" data-a-dec="," data-a-sep="." id="diskon_nominal" value="<?php echo e($pembelian->diskon_nominal ?? '0'); ?>">
                    </div>
                </div>
            </div>
            <div class="form-group">
                ...
                <div style="display:flex">
                    <div class="input-group" style="width:150px">
                        <input type="text" name="ppn_persen" class="form-control autonumeric" data-a-dec="," data-a-sep="." id="ppn_persen" value="<?php echo e($pembelian->ppn_persen ?? '0'); ?>">
                        <div class="input-group-prepend">
                            <span class="input-group-text"><i class="mdi mdi-percent-outline"></i></span>
                        </div>
                    </div>
                    <div class="input-group">
                        ...
                        <input type="text" name="ppn_nominal" class="form-control autonumeric" data-a-dec="," data-a-sep="." id="ppn_nominal" value="<?php echo e($pembelian->ppn_nominal ?? '0'); ?>">
                    </div>
                </div>
            </div>
            <div class="form-group mb-3">
                <label>Biaya Tambahan</label>
                <input type="text" name="biaya_tambahan" class="form-control autonumeric" data-a-dec="," data-a-sep="." id="biaya_tambahan" value="<?php echo e($pembelian->biaya_tambahan ?? '0'); ?>">
            </div>
            <div class="form-group mb-0">
                <label>File Faktur</label>
                <input type="file" name="file_faktur" class="form-control" id="file_faktur">
                <?php if(($pembelian->file ?? '') != ''): ?>
                    <a href="<?php echo e(asset('storage/' . $pembelian->file)); ?>">Lihat File</a>
                <?php endif; ?>
            </div>
            <hr>
            ...
            <div class="position-absolute d-flex flex-column" style="bottom: 20px;width: calc(100% - 40px);gap: 10px;">
                <a href="<?php echo e(url('manajemen_stok/pembelian')); ?>" class="btn btn-secondary flex-grow-1" >Kembali</a>
...
<?php $__env->startSection('js'); ?>
    ...
    <script>
        let _token = '<?php echo e(csrf_token()); ?>', sub_total = 0, no_pembelian = '', diskon = 0, ppn = 0, total = 0, tambahan = parseInt('<?php echo e($pembelian->biaya_tambahan ?? '0'); ?>'),
        ...
        let open_modal_tambah_produk = () => {
            $.get("<?php echo e(url('manajemen_stok/pembelian_baru/item/produk_create')); ?>", (result) => {
            ...
            return new Promise(async (resolve, reject) => {
                ...
                try {
                    const data = await $.ajax({
                        url: "<?php echo e(url('manajemen_stok/pembelian_baru/create')); ?>",
                        type: "POST",
                        data: params,
                        processData: false,
                        contentType: false,
        ...
        let search_items = () => {
            $.post("<?php echo e(url('manajemen_stok/pembelian_baru/item')); ?>/" + pembelian_id + '/search', {_token}, (result) => {
        ...
        let update_item = (id) => {
            if (loading === false) {
                ...
                $.ajax({
                    url: "<?php echo e(url('manajemen_stok/pembelian_baru/item')); ?>/" + id + '/update',
                    type: "POST",
                    data: formData,
                    processData: false,
                    contentType: false,
        ...
        let delete_pembelian = () => {
            if (loading === false) {
                loading = true;
                if (pembelian_id !== '') {
                    $.post("<?php echo e(url('manajemen_stok/pembelian_baru')); ?>/" + pembelian_id + '/delete', {_token}, () => {
                        loading = false;
                        window.location.href = '<?php echo e(url('manajemen_stok/pembelian_baru')); ?>';
                    });
                }
            }
        }
        ...
        let delete_item = (id) => {
            if (loading === false) {
                loading = true;
                $.post("<?php echo e(url('manajemen_stok/pembelian_baru/item')); ?>/" + id + '/delete', {_token}, () => {
                    loading = false;
                    search_items();
                }).fail((xhr) => {
                    loading = false;
        ...
        let search_produk = () => {
            ...
            $.post("<?php echo e(url('manajemen_stok/pembelian_baru/cari_produk')); ?>", {_token, nama}, (result) => {
        ...
        let selesai_pembelian = () => {
            if (loading === false) {
                ...
                $.ajax({
                    url: "<?php echo e(url('manajemen_stok/pembelian_baru')); ?>/" + pembelian_id + '/update',
                    type: "POST",
                    data: formData,
                    processData: false,
                    contentType: false,
                    ...
                    .done(function(data) {
                        loading = false;
                        window.location.href = '<?php echo e(url('manajemen_stok/pembelian_baru/selesai')); ?>?id=' + pembelian_id;
                        // Handle success response here
                    })
                    .fail(function(xhr) {
                        loading = false;
        ...
        $fid_supplier.change(() => {
            if (loading === false) {
                let fid_supplier = $fid_supplier.find('option:selected').val();
                $.post("<?php echo e(url('manajemen_stok/pembelian_baru')); ?>/" + pembelian_id + '/update', {
                    _token,
                    fid_supplier
                });
            }
        ...
        $kode_produk.change(async () => {
            if (loading === false) {
                ...
                $.post("<?php echo e(url('manajemen_stok/pembelian_baru/item/create')); ?>", {
                    ...
                    if (result.error) swal.fire(result.error);
                        ...
                        $kode_produk.val('');
                        if (is_baru === true) {
                            window.location.href = '<?php echo e(url('manajemen_stok/pembelian_baru')); ?>?no_pembelian=' + no_pembelian + '&id=' + pembelian_id;
                        } else search_items();
        ...
        <?php if(!empty($pembelian)): ?>
            pembelian_id = '<?php echo e($pembelian->id); ?>';
            search_items();
        <?php endif; ?>
...
<?php echo $__env->make('layouts.kasir2', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH /var/www/html/resources/views/pos/pembelian_baru/index.blade.php ENDPATH**/ ?>