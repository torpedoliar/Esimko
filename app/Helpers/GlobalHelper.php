...
class GlobalHelper
{
    ...
    public static function saldo_simpanan($list_fid_anggota) {
        $data = Transaksi::select('fid_anggota', 'fid_jenis_transaksi', DB::raw('SUM(nominal) as nominal'))
            ...
            ->whereIn('fid_jenis_transaksi', [1, 2, 3, 4, 5, 6, 7])
            ->where('fid_status', 4)
            ->groupBy(['fid_anggota', 'fid_jenis_transaksi'])
            ->get();
        $result = [];
        foreach ($data as $value) $result[strtoupper($value->fid_anggota . '_' . $value->fid_jenis_transaksi)] = $value->nominal;
        return $result;
    }
    ...
    {
        $periode = GlobalHelper::get_bulan($bulan_awal);
        $tenor = $tenor - 1;
    ...
    public static function get_range_periode($periode)
    {
        ...
        $periode = GlobalHelper::get_bulan($periode);
        $data = '';
        for ($q = 11; $q >= 0; $q--) {
    ...
    public static function saldo_tabungan($anggota, $jenis)
    {
        ...
        if ($jenis != 'all') {
            if ($jenis == 'Total Simpanan') {
                ...
                $query = $query->whereIn('fid_jenis_transaksi', array(3, 5, 6));
            } elseif ($jenis == 'Simpanan Hari Raya') {
                $query = $query->whereIn('fid_jenis_transaksi', array(4, 7));
            } else {
                $query = $query->where('fid_jenis_transaksi', $jenis);
            }
        }
//        if ($jenis === 2) dd($query->get());
        $saldo = $query->sum('nominal');

        return intval($saldo);
    }

    public static function detail_tabungan($anggota, $jenis, $tanggal_awal, $tanggal_akhir)
    {
        $query = Transaksi::where('fid_status', 4);
        if ($anggota !== '') $query = $query->where('fid_anggota', $anggota);
        ...
        if ($jenis != 'all') {
            if ($jenis == 'Total Simpanan') $query = $query->whereIn('fid_jenis_transaksi', array(1, 2, 3, 4, 5, 6, 7, 8));
            elseif ($jenis == 'Simpanan Sukarela') $query = $query->whereIn('fid_jenis_transaksi', array(3, 5, 6));
            elseif ($jenis == 'Simpanan Hari Raya') $query = $query->whereIn('fid_jenis_transaksi', array(4, 7));
            else $query = $query->where('fid_jenis_transaksi', $jenis);
        }
        return $query->orderBy('id')->with(['operator', 'status', 'anggota'])->get();
    }
    ...
    {
        $bulan = GlobalHelper::dateFormat($tanggal, 'Y-m');
        ...
        return $kode . '-' . GlobalHelper::dateFormat($tanggal, 'm-Y') . '-' . $no_urut;
    }
...
//        $last = Pembelian::where()
        ...
        $pembelian = DB::table('pembelian')->whereMonth('created_at', $bulan)->whereYear('created_at', $tahun)->orderBy('created_at', 'DESC')->first();
        if (!empty($pembelian)) {
    ...
    public static function bulan_payroll($jenis)
    {
        ...
        $bulan = GlobalHelper::get_bulan($bulan_terakhir);
    ...
    public static function stok_barang($id, $penjualan = 'all')
    {
        ...
        if (!empty($produk)) {
            $pembelian = ItemPembelian::where('fid_produk', $id)
                ->whereHas('pembelian', function ($pembelian) {
                    $pembelian->where('status', 1);
                })
                ->where('fid_pembelian', '<>', 0)->sum('jumlah');
            $return_pembelian = ItemReturPembelian::where('fid_produk', $id)->sum('jumlah');
            ...
            $data = [
                ...
                'sisa' => $sisa,
                'penyesuaian' => intval($penyesuaian)
            ];
        } else {
