<table class="table table-sm table-bordered">
    ...
    @foreach($list_akun as $value)
        ...
        <tr>
            ...
            <td class="text-right">{{ format_number(substr($value->kode_tampil, 0, 1) == '3' ? (-1 * $value->nominal) : $value->nominal) }}</td>
        </tr>
    @endforeach
    <tr class="border-top">
        <td><b>Total Pengeluaran</b></td>
        <td class="text-right">{{ format_number($pengeluaran) }}</td>
    </tr>
    <tr>
        <td colspan="2"></td>
    </tr>
    <tr class="border-top">
        <td><b>Laba/rugi</b></td>
        <td class="text-right">{{ format_number($pendapatan - $pengeluaran) }}</td>
    </tr>
    </tbody>
</table>
