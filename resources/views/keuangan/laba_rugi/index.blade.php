...
@section('js')
    <script>
        ...
        let search = () => {
            ...
            $.post("{{ url('keuangan/laba_rugi/search') }}", {_token, bulan, bulan_akhir}, (result) => {
                $table.html(result);
            }).fail((xhr) => {
        ...
        let export_excel = () => {
            let bulan = $('#bulan').val();
            let bulan_akhir = $('#bulan_akhir').val();
            window.open("{{ url('keuangan/laba_rugi/export') }}?bulan=" + bulan + '&bulan_akhir=' + bulan_akhir, '_blank');
        }
