    ...
    <div class="container-fluid">
      ...
      <form action="{{url('main/transaksi/filter')}}" method="post" id="filter_transaksi" class="mt-5">
        ...
        <div class="row">
          ...
          <div class="col-md-2">
            <a href="{{url('main/pinjaman/form')}}" class="btn btn-primary btn-block">Formulir Pinjaman</a>
          </div>
        </div>
                  ...
                  <tr>
                    ...
                    <td style="width:1px;white-space:nowrap">
                      <div class="text-center">
                        <a href="{{url('main/pinjaman/detail?id='.$value->id)}}" class="text-dark"><i class="bx bx-search-alt h3 m-0"></i></a>
                      </div>
                    </td>
                  </tr>
                @endforeach
        ...
        <div style="border-left:1px solid #dedede;padding:20px 20px;height:100%;width:250px" class="mb-4">
          @foreach ($data['jenis-transaksi'] as $key => $value)
            <div class="form-group">
              <label>Sisa {{$value->jenis_transaksi}}</label>
              <div class="font-size-13">Rp {{number_format($value->sisa_pinjaman,0,'.',',')}}</div>
            </div>
          @endforeach
          <div class="form-group">
            <label>Total Sisa Pinjaman</label>
            <div class="font-size-13">Rp {{number_format($data['total-sisa'],0,'.',',')}}</div>
          </div>
        </div>
      </div>
    </div>
