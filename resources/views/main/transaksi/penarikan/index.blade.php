      ...
      <div class="media">
        ...
        <div class="media-body align-self-center">
          ...
          <ol class="breadcrumb m-0">
            ...
            <li class="breadcrumb-item"><a href="#">Penarikan Simpanan</a></li>
  ...
  <div class="row">
        ...
        <div class="card-body">
          ...
          <form action="{{url('transaksi/proses')}}" method="post">
            ...
            <div class="form-group">
              ...
              <div style="text-align: right;font-style: italic;color: #a7a7a7;margin-top: 5px;">
    ...
    <div class="col-md-8">
      <div class="card">
        <div class="card-header p-3">
          <form action="{{url('transaksi/filter')}}" method="post" id="filter_transaksi">
            ...
            <div class="row">
              <div class="col-md-4">
                <input type="hidden" id="status_id" name="status" value="{{(!empty($filter['penarikan']) ? $filter['penarikan']['status'] : 'all' )}}">
                <select class="select2-status" id="status_color" style="width:100%" onchange="pilih_status()">
                  ...
                  <option value="{{$value->color}}" {{(!empty($filter['penarikan']) ? ($filter['penarikan']['status']==$value->id ? 'selected' : '') : '' )}}  data-id="{{ $value->id}}" >{{$value->status}}</option>
                  @endforeach
                </select>
              </div>
              <div class="col-md-6">
                <div>
                  <div class="input-daterange input-group" data-date-format="dd-mm-yyyy" data-provide="datepicker">
                    <input type="text" class="form-control" value="{{(!empty($filter['penarikan']) ? $filter['penarikan']['from'] : '' )}}" autocomplete="off" id="from" onchange="javascript:submit()" name="from" placeholder="Dari Tanggal" />
                    <input type="text" class="form-control" value="{{(!empty($filter['penarikan']) ? $filter['penarikan']['to'] : '' )}}" autocomplete="off" id="to" onchange="javascript:submit()" name="to" placeholder="Sampai Tanggal" />
                  </div>
                </div>
              </div>
            </div>
      ...
      <div style="width:100%;text-align:center" class="mb-5">
        ...
        <h5 class="mt-3">Data Penarikan Simpanan Tidak Ditemukan</h5>
      </div>
      @else
        ...
        <table class="table table-middle table-custom">
          <thead class="thead-light">
            <tr>
              ...
              <th style="text-align:right">Jumlah Penarikan</th>
              <th></th>
            </tr>
          </thead>
          <tbody>
            @foreach ($data['penarikan'] as $key => $value)
              <tr>
                ...
                <td style="text-align:right">Rp {{number_format(str_replace('-','',$value->nominal),0,',','.')}}</td>
                <td style="width:1px;white-space:nowrap">
                  <div class="text-center">
                    <a href="{{url('penarikan/detail?id='.$value->id)}}" class="text-dark"><i class="bx bx-search h3 m-0"></i></a>
                  </div>
                </td>
              </tr>
            @endforeach
