...
<table class="table table-middle table-custom">
        ...
        @if($temp != $value->pembelian->fid_supplier)
            @if($total_before > 0)
                <tr>
                    ...
                    <td style="text-align: right;"><b>{{ ($total_before - $sub_diskon + $sub_ppn) }}</b></td>
                    <td></td>
                </tr>
    ...
    <tr>
        <td colspan="6" style="text-align: right;"><b>Sub Total</b></td>
        <td style="text-align: right;"><b>{{ ($total_before - $sub_diskon + $sub_ppn + $sub_biaya) }}</b></td>
        <td></td>
    </tr>
    @php($total_diskon += $sub_diskon)
    @php($total_ppn += $sub_ppn)
    ...
    <tr>
        ...
        <td style="text-align:right"><b>{{ ($pembelian->sum('total') - $total_diskon + $total_ppn + $total_biaya) }}</b></td>
        <td></td>
    </tr>
    </tbody>
</table>
