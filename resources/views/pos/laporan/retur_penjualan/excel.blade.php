...
<table class="table table-bordered table-sm">
    ...
    @foreach ($retur_penjualan as $key => $value)
        @if($temp != $value->retur_penjualan->fid_anggota)
            ...
            @endif
            <tr>
                <td colspan="8"><b>{{ $value->retur_penjualan->fid_anggota ?? '' }} - {{ $value->retur_penjualan->anggota->nama_lengkap ?? '' }}</b></td>
            </tr>
            @php($total_before = 0)
        @endif
        <tr>
            ...
            <td style="text-align:right">{{ ($value->produk->harga_jual) }}</td>
            <td style="text-align:right">{{ ($value->jumlah) }}</td>
            <td style="text-align:right">{{ ($value->produk->harga_jual * $value->jumlah) }}</td>
