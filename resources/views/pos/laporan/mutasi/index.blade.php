...
@section('js')
    <script>
        ...
        let search_data = (page = 1) => {
            ...
            data.paginate = 10;
            data.page = page;
            $.post("{{ url('pos/laporan_mutasi/search') }}", data, (result) => {
                $table.html(result);
            }).fail((xhr) => {
        ...
        let export_Excel = () => {
            ...
            window.open("{{ url('pos/laporan_mutasi/excel') }}?" + data, '_blank');
        }

        let cetak = () => {
            let data = $form_search.serialize();
            window.open("{{ url('pos/laporan_mutasi/cetak') }}?" + data, '_blank');
        }
