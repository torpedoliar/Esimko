...
<div class="container-fluid">
  <div class="content-breadcrumb mb-2">
    ...
    <div class="row">
      ...
      <div class="col-md-2">
        <a href="{{url('pos/belanja_konsinyasi/form')}}" class="btn btn-primary btn-block" >Tambah Transaksi</a>
      </div>
    </div>
  </div>
  ...
  <div style="width:100%;text-align:center">
    ...
    <h4 class="mt-2">DATA BELANAJ KONSINYASI TIDAK DITEMUKAN</h4>
  </div>
  @else
    ...
    <table class="table table-middle table-custom">
      <thead class="thead-light">
        <tr>
          ...
          <th>Anggota Transaksi</th>
          <th>No. Transaksi / Nama Toko</th>
          <th style="text-align:right">Total<br>Belanja</th>
          <th class="center" style="width:50px">Margin</th>
          <th class="center" style="width:50px">Tenor</th>
      ...
      <tbody>
        ...
        <tr>
          <td class="center" style="width:150px;border-color:{{$value->color}}">{{\App\Helpers\GlobalHelper::dateFormat($value->tanggal,'d/m/Y')}}</td>
          <td>
            <div class="media">
          ...
          <td style="text-align:right">{{number_format($value->total_pembayaran,0,',','.')}}</td>
          <td class="center">{{$value->margin}}%</td>
          <td class="center">{{$value->tenor}}<br>bulan</td>
          <td style="text-align:right">{{number_format($value->angsuran,0,',','.')}}</td>
          <td style="width:1px;white-space:nowrap">
            <div class="text-center">
              <a href="{{url('pos/belanja_konsinyasi/detail?id='.$value->id)}}" class="text-dark"><i class="bx bx-search-alt h3 m-0"></i></a>
            </div>
          </td>
        </tr>
        @endforeach
