<div class="table-responsive mt-4 mb-4">
    <table class="table table-middle table-custom">
        <thead>
        <tr>
            <th>No. Pembelian<hr class="line-xs">Waktu</th>
            <th>Pembeli</th>
        ...
        <tbody>
        @foreach ($list_pembelian as $key => $value)
            <tr onclick="resume_pembelian('{{ $value->no_transaksi }}')">
                ...
                <td class="center">{{$value->items->sum('jumlah')}}</td>
                <td class="center">{{$value->metode_pembayaran->keterangan}}</td>
                <td style="text-align:right;white-space:nowrap">Rp {{number_format($value->items->sum('total'),0,',','.')}}</td>
                ...
                <td style="width:1px;white-space:nowrap">
                    <a href="javascript:void(0)" class="text-dark"><i class="bx bx-right-arrow h3 m-0"></i></a>
                </td>
            </tr>
        @endforeach
        </tbody>
